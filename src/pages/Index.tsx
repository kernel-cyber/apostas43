        ...
        <div className="relative container mx-auto px-2 sm:px-4 py-12 sm:py-16 md:py-20 text-center">
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 mt-8 sm:mt-12 max-w-4xl mx-auto px-2">
              {[
                { label: "Seus Pontos", value: points.toLocaleString(), icon: Star, color: "text-accent" },
                { label: "Pool Total", value: `${(stats.totalPool / 1000).toFixed(1)}K`, icon: Trophy, color: "text-primary" },
                { label: "Apostas", value: stats.totalBets.toString(), icon: TrendingUp, color: "text-neonGreen" },
                { label: "Pilotos", value: stats.activePilots.toString(), icon: Crown, color: "text-chrome" }
              ].map(({ label, value, icon: Icon, color }, i) => (
                <div key={i} className="glass-card p-3 sm:p-4 rounded-xl text-center card-enter" style={{ animationDelay: `${i * 0.1}s` }}>
