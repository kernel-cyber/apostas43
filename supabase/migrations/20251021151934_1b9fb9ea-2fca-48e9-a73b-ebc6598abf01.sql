-- Add cycle_position column to matches table
ALTER TABLE matches ADD COLUMN IF NOT EXISTS cycle_position INTEGER;

-- Drop and recreate generate_top20_matches function with cycle_position
...
CREATE OR REPLACE FUNCTION public.generate_top20_matches(p_event_id uuid, p_bracket_type text)
 RETURNS TABLE(round_num integer, pilot1_id uuid, pilot2_id uuid, pilot1_pos integer, pilot2_pos integer, cycle_pos integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Contar quantas rodadas odd e even já existem
  SELECT COUNT(DISTINCT round_number)
  INTO v_odd_count
  FROM matches
  WHERE event_id = p_event_id AND bracket_type = 'odd';

  SELECT COUNT(DISTINCT round_number)
  INTO v_even_count
  FROM matches
  WHERE event_id = p_event_id AND bracket_type = 'even';

  -- Calcular cycle_position
  -- 1 = 1ª Ímpar, 2 = 1ª Par, 3 = 2ª Ímpar, 4 = 2ª Par
  IF v_bracket_suffix = 'odd' THEN
    IF v_odd_count = 0 THEN
      v_cycle_position := 1; -- 1ª Rodada Ímpar
    ELSIF v_odd_count = 1 THEN
      v_cycle_position := 3; -- 2ª Rodada Ímpar
    ELSE
      -- Reinicia ciclo (pode haver mais de 4 etapas em eventos longos)
      v_cycle_position := CASE WHEN v_odd_count % 2 = 0 THEN 1 ELSE 3 END;
    END IF;
  ELSE -- even
    IF v_even_count = 0 THEN
      v_cycle_position := 2; -- 1ª Rodada Par
    ELSIF v_even_count = 1 THEN
      v_cycle_position := 4; -- 2ª Rodada Par
    ELSE
      -- Reinicia ciclo
      v_cycle_position := CASE WHEN v_even_count % 2 = 0 THEN 2 ELSE 4 END;
    END IF;
  END IF;
